...
class SchemaModelTest extends TestCase {
    ...
    public function test_column_information() {
        ...
        $this->assertTrue(Schema::required('users', 'id'));
        $this->assertFalse(Schema::required('users', 'email_verified_at'));
        $required = Schema::required('users', 'unknown');
        $this->assertFalse($required); // non existing are not required

        $info = Schema::columnInformation("users", "password");

        // Commented out, I am not sure that field comments will stay the prefered way
        // to defined metadata
        ...
        $info = Schema::columnInformation("users", "unknown_key");
    ...
    public function test_attributes() {
        ...
        $this->assertEquals(0, Schema::columnSize('users', 'created_at'));
